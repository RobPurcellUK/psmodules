...
.DESCRIPTION
   ...
   Original cowsay (c) 1999-2000 Tony Monroe.
    ...
    Module conversion by Rob Purcell 2017
.EXAMPLE
   Write-CowSay "Well, hello there! Mooo!"
#>
...
function slurp_input($in, $ar) {
    if(!$ar) {
        ...
        if($opts.n) { display_usage; return }
        [string]::join(' ', ($ar | % {
...
function get_cow($f, $path, $vars) {
    ...
    $fpath = "$path\$f"
    if(!(test-path $fpath)) { "$script:progname: could not find $f cowfile!"; return }
    $script = gc -raw $fpath 
...
function Write-CowSay() {
    ...
    try {
        ...
        "$progname`: $_"; display_usage; return;
    }

    if($opts.h) { display_usage; return }
    if($opts.l) { list_cowfiles $cowpath; return }
    ...
    $balloon_lines, $thoughts = construct_balloon $message ($progname -eq 'Write-CowThink')
...
function Write-CowThink() {
    if($myinvocation.expectingInput) { $input | & Write-CowSay @args } else { & Write-CowSay @args }
}

Set-Alias -Name cowsay -Value Write-CowSay
Set-Alias -Name cowthink -Value Write-CowThink

Export-ModuleMember -Function Write-CowSay,Write-CowThink -Alias cowsay,cowthink
